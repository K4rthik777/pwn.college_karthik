...
### Solve
**Flag:** `pwn.college{EhxuAmmH68vKzhbr_cdb9h77ZQm.QX1UDN1wSN1gjNzEzW}`
 for this Challenge, i ran su command and then enter the root password and got the root access.Then catted the /flag to get the flag.

```bash 
hacker@users~becoming-root-with-su:~$ su
Password:
root@users~becoming-root-with-su:/home/hacker# cat /flag
pwn.college{EhxuAmmH68vKzhbr_cdb9h77ZQm.QX1UDN1wSN1gjNzEzW}
```

### New Learnings
I learned how su command works to grant the user root access by after verifying with root password.

### References

## Others userd with su
...
### Solve
**Flag:** `pwn.college{IWQdOKT1LzgRxX5YM_TvN58NTYP.QX2UDN1wSN1gjNzEzW}`
 for this Challenge, i ran su command with zardus as argument and then enter the user password and got the user access as zardus.Then ran /challenge/run to get the flag.
...
pwn.college{IWQdOKT1LzgRxX5YM_TvN58NTYP.QX2UDN1wSN1gjNzEzW}
```

### New Learnings
I learned how su command with username as argument works to grant that user access by after verifying with root password.
...
### Solve
**Flag:** `pwn.college{wsRRcB7rZ8OvEwW67nADg1hBotI.QX3UDN1wSN1gjNzEzW}`
 for this Challenge, i ran john command with /challenge/shadow-leak as argument and then i got the password of zardus.Then i ran su command with zardus and then enter the user password and got the user access as zardus.Then ran /challenge/run to get the flag.
...
Press 'q' or Ctrl-C to abort, almost any other key for status
aardvark         (zardus)
1g 0:00:00:18 100% 2/3 0.05420g/s 315.6p/s 315.6c/s 315.6C/s Johnson..buzz
Use the "--show" option to display all of the cracked passwords reliably
Session completed
...
Congratulations, you have become Zardus! Here is your flag:
pwn.college{wsRRcB7rZ8OvEwW67nADg1hBotI.QX3UDN1wSN1gjNzEzW}
```

### New Learnings
I learned how john command with the leaked password hash file as argument to get the passwords.
...
### Solve
**Flag:** `pwn.college{cCHDwZ3O1p3LDzrWWHszKCAQ0M5.QX4UDN1wSN1gjNzEzW}`
 for this Challenge, i ran /challenge/run with sudo prepended to get the flag.

```bash 
hacker@users~using-sudo:~$ sudo cat /flag
pwn.college{cCHDwZ3O1p3LDzrWWHszKCAQ0M5.QX4UDN1wSN1gjNzEzW}
```

### New Learnings
I learned how sudo prepended to any command if it has authorization as per establised policies in /etc/sudoers runs the command in root shell .
